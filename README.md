...
**V1 核心工作流：**
1. Agent 通过 `POST /v1/agents` 向 Registry 注册（可传入含 `security_schemes` 的 AgentCard）
2. 认证启用时，注册成功自动获取 OAuth 2.1 client_id/client_secret
3. Agent 通过 WebSocket (`/v1/agents/{id}/ws`) 建立长连接，或通过 HTTP 心跳保持活跃
4. 客户端通过 `POST /v1/agents/{id}/dispatch` 向已连接的 Agent 分发任务
5. Agent 通过 WebSocket 接收任务、处理、返回结果
6. 客户端通过 `GET /v1/tasks/{id}` 轮询任务状态和结果
...
### 注册 Agent + 获取 Token（认证模式）

```bash
# 1. 注册 Agent（传入含 OAuth2SecurityScheme 的 AgentCard）
...
# 响应会包含 client_id 和 client_secret，用于获取 Token
...
TOKEN=$(curl -s -X POST http://localhost:8321/auth/token \
  ...
  -d "scope=task:read task:write agent:read" | jq -r '.access_token')

# 3. 使用 Token 调用受保护 API
