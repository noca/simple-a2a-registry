...
> 开启 `--auth-enabled true` 后，受保护端点需要 `Authorization: Bearer <token>` header。
> 详见下方 [OAuth 2.1 认证端点](#oauth-21-认证端点) 章节。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `grant_type` | string | 是 | 授权类型：`client_credentials` / `authorization_code` |
| `client_id` | string | 是 | 客户端 ID（Agent 注册时获得） |
| `client_secret` | string | 是 | 客户端密钥 |
| `scope` | string | 否 | 请求的权限范围（空格分隔，如 `task:read task:write`） |
| `code` | string | 否 | Authorization Code（`grant_type=authorization_code` 时必填） |
...
| 400 | `invalid_scope` | 请求的 scope 超出允许范围 |
...
返回 RFC 8414 标准的 OAuth 2.0 Authorization Server 元数据。
...
```json
{
  ...
  "authorization_endpoint": "http://localhost:8321/auth/authorize",
  "token_endpoint": "http://localhost:8321/auth/token",
  "registration_endpoint": "http://localhost:8321/auth/register",
  "jwks_uri": "http://localhost:8321/.well-known/jwks.json",
  "scopes_supported": [
    "task:read", "task:write",
    "agent:read", "agent:register",
    "agent:admin", "registry:admin"
  ],
...
| Scope | 描述 | 适用端点 |
|-------|------|---------|
| `task:read` | 读取任务列表和详情 | `GET /v1/tasks`, `GET /v1/tasks/{id}`, V2 任务查询 |
| `task:write` | 创建和修改任务 | `POST /v1/agents/{id}/dispatch`, V2 任务操作 |
...
|------|------|------|------|-------|
| GET | `/health` | 健康检查 | ❌ 公开 | — |
| GET | `/.well-known/agent-card.json` | Registry 自身的 A2A Agent Card | ❌ 公开 | — |
| GET | `/v1/agents` | 列出/搜索 Agent | ✅ | `agent:read` |
| GET | `/v1/agents/{agent_id}` | 获取 Agent 详情 | ✅ | `agent:read` |
| POST | `/v1/agents` | 注册 Agent | ✅ | `agent:register` |
| DELETE | `/v1/agents/{agent_id}` | 注销 Agent | ✅ | `agent:admin` |
| POST | `/v1/agents/{agent_id}/heartbeat` | Agent 心跳保活 | ✅ | `agent:read` |
| **GET** | **`/v1/agents/{agent_id}/ws`** | **Agent WebSocket 长连接** | ✅ | `agent:read` |
| **POST** | **`/v1/agents/{agent_id}/dispatch`** | **通过 WS 分发任务到 Agent** | ✅ | `task:write` |
...
| GET | `/v2/tasks/{id}` | 任务详情（含依赖链、评论、运行历史、事件流） | 200 | ✅ | `task:read` |
| POST | `/v2/tasks/{id}/claim` | Worker 认领任务 | 200 | ✅ | `task:write` |
...
| POST | `/v2/tasks/{id}/heartbeat` | 任务级心跳（延长认领锁 TTL） | 200 | ✅ | `task:write` |
...
| GET | `/v2/stats` | 编排引擎统计 | 200 | ✅ | `task:read` |
