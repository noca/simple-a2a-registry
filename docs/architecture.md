...
### Agent 注册 → Token → 认证请求（方案C：Admin 预创建）

```
1. Admin 预创建 OAuth 客户端
   │
   ├── 调用 POST /admin/clients（需 registry:admin scope）
   │   └── 或通过 Web UI Dashboard 创建
   │
   ├── Registry 生成 client_id + client_secret
   └── Admin 将凭证通过安全渠道分发给 Agent
   │
   ▼
2. Agent 启动
   │
   ├── 加载 Admin 预分配的 client_id + client_secret
   │
   ▼
3. POST /auth/token (client_credentials)
   │
   ├── Registry 签发 JWT access_token
   └── 返回 Bearer token（有效期 1 小时，附带 agent:register scope）
   │
   ▼
4. POST /v1/agents 注册（带 Authorization: Bearer ***）
   │
   ├── Registry 验证 Token + agent:register scope
   ├── 注册 Agent 到 Registry（不再自动创建 OAuth 客户端）
   └── 返回 agent_id
   │
   ▼
5. GET /.well-known/agent-card.json
   │
